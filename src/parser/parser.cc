...
Parser::ErrorOrPtr<ast::BlockStatement> Parser::parse_statement_as_list() {
  auto location = scoped_location();
  ...
  if (current_token().type() == TokenType::OPEN_BRACE) {
    ...
    block = std::make_unique<ast::BlockStatement>(
        location.range(),
...
Parser::ErrorOrPtr<ast::IfStatement> Parser::parse_if_statement() {
  ...
  RETURN_OR_MOVE(auto if_body, parse_statement_as_list());
  ...
  if (current_token().type() == TokenType::ELSE) {
    ...
    if (current_token().type() == TokenType::IF) {
      ...
      RETURN_OR_MOVE(auto else_body, parse_statement_as_list());
