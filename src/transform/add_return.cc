...
void VoidFunctionReturnAdder::visit(ast::IfStatement* node) {
  if (node->else_statement().is_ok()) {
    ...
    has_returned_ = false;
    ASTVisitor::visit(node->else_statement().value_or_die().get());
    has_returned_ &= if_returned;
  }
}
...
void VoidFunctionReturnAdder::visit(ast::FunctionDeclaration* node) {
  ...
  CHECK(node->type().is_ok()) << "Function return type not deduced: " << node->name();
  const auto& type = node->type().value_or_die();
  CHECK(type.is_resolved()) << "Function return type not resolved: " << node->name();
  ...
  if (has_returned_) {
    CHECK(!statements->statements().empty()) << "Empty function that returned: " << node->name();
  } else {
    if (type.get_declaration() == &ast::types::void_type) {
      statements->statements().emplace_back(
          std::make_unique<ast::ReturnStatement>(lexer::invalid_range(),
                                                 none));
    } else {
