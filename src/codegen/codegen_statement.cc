...
void CodeGenerator::visit(ast::IfStatement* node) {
  ...
  auto current_function = current_function_.value_or_die();
  ...
  BasicBlock* if_block =
      BasicBlock::Create(context_, "if.true", current_function);
  ...
  if (node->else_statement().is_ok()) {
    else_block = BasicBlock::Create(context_, "if.else", current_function);
  ...
  if (!has_returned_) {
    ifend_block = BasicBlock::Create(context_, "if.end", current_function);
  }
