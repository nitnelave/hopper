...
class CodeGenerator : public ast::ASTVisitor {
  ...
  using ScopedFunctionArgs = std::stack<std::unordered_set<std::string>>;
  using Functions =
      std::unordered_map<std::string, std::stack<llvm::Function*>>;
  using FunctionArgs = std::unordered_map<std::string, std::stack<llvm::Value*>>;
 ...
 private:
  ...
  void add_function_scope_level(const std::string& fun_name, llvm::Function* function);
  ...
  ScopedFunctionArgs scoped_fun_args_;
