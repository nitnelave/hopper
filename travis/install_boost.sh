...
if [[ "${BOOST_VERSION}" != "" ]]; then
  ...
  if [[ -z "$(ls -A ${BOOST_DIR} 2>/dev/null)" ]]; then
    if [[ "${BOOST_VERSION}" == "trunk" ]]; then
      ...
      (cd ${BOOST_DIR} && ./bootstrap.sh && ./b2 headers --toolset=clang)
    else
  ...
  (cd ${BOOST_DIR} && ./bootstrap.sh --prefix=${INSTALL_DIR} && ./b2 install --toolset=clang -q -d0 --prefix=${INSTALL_DIR} --with-program_options -j${JOBS})
